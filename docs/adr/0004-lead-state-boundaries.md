...
>
> **2026-07-19 — status-model deferral lifted by [ADR-0013](0013-lead-crm-view-and-status-taxonomy.md).** The `preMQL/MQL/SQL/won/lost/lost_premql` taxonomy (`leads.crm_status`) is now in scope. The legacy booleans and `qualification` are retained and stay KPI-authoritative during migration; a DB trigger recomputes `meeting_booked`/`meeting_held`/`offer_sent` from the new `lead_meetings`/`lead_offers` child tables. `mapLeadPatch` remains the single whitelist for **direct** lead edits — the trigger only derives booleans, it does not widen the editable set.
