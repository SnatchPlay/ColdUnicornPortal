...
>
> **2026-07-19 — status-model deferral lifted by [ADR-0013](0013-lead-crm-view-and-status-taxonomy.md).** The `preMQL/MQL/SQL/won/lost/lost_premql` taxonomy is now in scope, modelled as a SPLIT: non-terminal `crm_stage` and `contact_disposition` (OOO/NRR) are DERIVED on read; only the explicit terminal `leads.final_outcome` (+ `conclusion`/`concluded_at`) is stored. Existing booleans and `qualification` are retained and stay **KPI-authoritative — the KPIs are not migrated**. A DB trigger recomputes `meeting_booked`/`meeting_held`/`offer_sent` from the new `lead_meetings`/`lead_offers` child tables. `mapLeadPatch` remains the single whitelist for **direct** lead edits; `final_outcome` is written only by the Phase-5 conclusion action, not free-edit.
