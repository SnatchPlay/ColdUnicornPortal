...
| [`searchable-select.tsx`](../src/app/components/searchable-select.tsx) | **`SearchableSelect`** — single-select where **the trigger becomes the search field on open** (one row, no extra search input; autofocus, ↑/↓ + Enter, Escape; trigger label comes from `value`, so filtering never blanks it). Use it for any list too long to scan — clients, campaigns. Built on `Popover` + `PopoverAnchor`, **not** `Select`: Radix's Select owns the keyboard for its own typeahead and its trigger cannot become an input. Do not bolt a search `<input>` next to — or under — a `<Select>` |
...
| Thing | Where |
|---|---|
| Role scoping | [`lib/selectors.ts`](../src/app/lib/selectors.ts) — `scopeClients`, `scopeCampaigns`, `scopeLeads`, `scopeReplies`, `scopeCampaignStats`, `scopeDailyStats`, `scopeDomains`, `scopeEmailAccounts` (mailboxes via `domain → client`), `scopeInvoices`, `getLeadStage`, `getRoleLabel`, `isInternalAdmin`, `sortClientsAlpha` (A→Z for **every** client dropdown — don't hand-roll a comparator) |
...
| Paged list | [`components/list-pagination.tsx`](../src/app/components/list-pagination.tsx) — **`ListPagination`**, the prev / first+ellipsis / window / ellipsis+last / next control used by the leads grid and the Team users list. Clamp the page with `clampPage` from [`lib/pagination.ts`](../src/app/lib/pagination.ts) and pass it in; the page window is the component's business. Don't rebuild the pager markup on a page |
| Timeframes | [`lib/timeframe.ts`](../src/app/lib/timeframe.ts) — `TimeframeValue`, `TIMEFRAME_PRESETS`, `DEFAULT_TIMEFRAME_PRESET`, `LEADS_DEFAULT_TIMEFRAME_PRESET` (All time — the Leads tab's own default), `createDefaultTimeframe` (takes an optional preset), `filterByTimeframe`, `resolveTimeframeBounds`, `getTimeframeLabel`, `normalizeTimeframePreset` (degrade a retired or bookmarked preset to the default, or to a per-page fallback passed as the 2nd argument — **never** switch on `timeframe.preset` yourself) |
