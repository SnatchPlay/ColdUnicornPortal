...
- **Portal tooltip** (`PORTAL_CHART_TOOLTIP`) — used by client-portal charts. Background `#080808`, border `#242424`, border-radius 12px, text white. Label color `#a3a3a3`.
- **Internal tooltip** (`TOOLTIP` object in admin/manager pages) — background `rgba(2,6,23,0.98)`, border `rgba(148,163,184,0.2)`, border-radius 16px, `cursor: false`.
...
- **Type:** `BarChart`
- **Data:** `getDailySentSeries(timeframeStats)` → `Array<{ date, label, sent }>`
...
- **Type:** `BarChart`
- **Data:** inline — group timeframe leads by ISO-week start, count MQLs.
...
- **Type:** `BarChart`
- **Data:** inline — group sorted scoped `daily_stats` by month, sum `mql_count`.
...
- **Type:** `BarChart`
- **Data:** inline — consecutive-day delta of `daily_stats.prospects_count`, last 10 days.
...
- **Type:** `BarChart`
- **Data:** inline — monthly sums of `campaign_daily_stats.sent_count`, last 3 calendar months.
- **Series:** `<Bar dataKey="sent" fill="#22c55e" />`
...
- **Type:** `BarChart`
- **Data:** monthly aggregation of `daily_stats.prospects_count` → month-over-month delta, last 12 months.
- **Series:** `<Bar dataKey="prospectsAdded" fill="#22c55e" />`
...
- **Data:** inline per-week last 8 weeks — `{ week, label, emailsDelta, mqls }`.
- **Series:**
...
- **Data:** `getPipelineActivitySeries(timeframeLeads)` → `Array<{ label, mqls, meetings, won }>`
- **Series:**
...
See [§8.2](#82-conversion-funnel-html-bars).
...
- Assigned client portfolio — tabular with inline KPI progress bars (HTML `<div>` widths, not recharts).
- Lead queue — tabular.

No charts to catalogue here beyond the HTML progress bars already described in [06 §1.4](./06-manager-portal.md#14-assigned-client-portfolio-surface).
...
- **Type:** `LineChart` (4 lines)
- **Data:** inline aggregation of `filteredStats` by `report_date` → `{ label, sent, replies, opens, bounces }`.
- **Series:**
...
- **Data:** inline — group `filteredLeads` by `qualification`, `{name, value}`.
...
- **Type:** `LineChart` (4 series: `sent`, `replies`, `opens`, `bounces`) — identical configuration to Client Campaigns daily volume chart [§3.1](#31-daily-campaign-volume-id-cc-daily-volume).
...
### 7.1 Campaign momentum

- **Type:** `AreaChart` (3 overlapping areas)
...
### 7.2 Non-active clients (formerly "At-risk") & Manager capacity

Tabular surfaces; no charts.
...
- Data per card: see [04-metrics §7](./04-metrics-catalog.md#7-client-dashboard-sparklines).

### 8.2 Conversion Funnel (HTML bars)

Rendered by `ClientDashboardPage` and `ClientStatisticsPage`. No recharts involvement. Each stage from `getConversionRates(…)` becomes a row:

- Label
- Value (formatted count)
- Rate label ("← X% prospect→MQL", etc.)
- Horizontal bar whose width encodes `value / from` with the stage color.

Colors per stage: Prospects `#3b82f6`, MQLs `#8b5cf6`, Meetings `#a855f7`, Won `#22c55e` ([04-metrics §3](./04-metrics-catalog.md#3-conversion-funnel)).
...
- Green fill when ≥ 100%, amber when 50–99%, red when below 50% (threshold constants inline in the page).

### 8.4 Pipeline badges & filter chips

Rendered by `PipelineBadge` and `FilterChip` in [`portal-ui.tsx`](../../../src/app/components/portal-ui.tsx). Visual only — dot color per `PIPELINE_STAGES` entry, count on chip.
...
|---------|-----------|--------|-------|
| Portal (client pages) | `#080808` | `#242424` | — |
