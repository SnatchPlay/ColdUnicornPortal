...
Items that appeared in the **archived** specs (`docs/archive/MASTER_FUNCTIONAL_SPECIFICATION.md`, `PROJECT_SPEC.md`) or in early scoping but are **explicitly not part of this product**. The canonical scope statement is in [BUSINESS_LOGIC.md §10](../../BUSINESS_LOGIC.md#10-out-of-scope-legacy); this file is the implementation-side companion.
...
|---|------|-------------|---------------|
| OoS-1 | **Health Assessments** (biweekly traffic-light form, `client_health_assessments` table, US-M3) | PROJECT_SPEC.md §6.2 | 2026-04-25 |
| OoS-2 | **CSV / Excel bulk import UI** (file picker, column mapper, validation preview, US-A10) | PROJECT_SPEC.md §6.3, §7.4 | 2026-04-25 |
| OoS-3 | **Cash flow projections** (`cash_flow_projections` table, US-A4) | PROJECT_SPEC.md §3.12, §6.3 | 2026-04-25 |
| OoS-4 | **Issue tracking per client** (`client_issues` table, US-M6) | PROJECT_SPEC.md §6.2 | 2026-04-25 |
| OoS-5 | **Auto-generated PDCA reports** (weekly/monthly CSV/PDF export, US-A7) | PROJECT_SPEC.md §6.3 | 2026-04-25 |
| OoS-6 | **Partnerships dashboard** (`partnerships` table) | PROJECT_SPEC.md §3.12 | 2026-04-25 |
| OoS-7 | **Lost-clients ABM tracking** (`abm_lost_clients` table, US-A8) | PROJECT_SPEC.md §3.13 | 2026-04-25 |
| OoS-8 | **Reply triage UI** (manual classification of replies) | inferred from `replies.classification` field | 2026-04-25 |
| OoS-9 | **In-portal email/SMS dispatch** (sending notifications from the portal directly) | PROJECT_SPEC.md §6.1 US-C8 | 2026-04-25 |
| OoS-10 | **RR/BR per-team-member reports** (US-A9) | PROJECT_SPEC.md §6.3 | 2026-04-25 |
| OoS-11 | **CRM platform plug-ins** (`client_crm_integrations` table for Pipedrive / HubSpot / Zoho / Salesforce / Livespace) | PROJECT_SPEC.md §3.3 | 2026-04-25 |
| OoS-12 | **Smartlead / Bison API sync from portal** (Phase 2 / future, §8.1-8.2) | PROJECT_SPEC.md §8 | 2026-04-25 |
| OoS-13 | **Account-Based Selling (ABS) scoring** (`account_based_selling` table) | PROJECT_SPEC.md §3.12 | 2026-04-25 |
| OoS-14 | **`crm_prospects` / `lg_pipeline` separate from `agency_crm_deals`** | PROJECT_SPEC.md §3.10-3.11 | 2026-04-25 |
| OoS-15 | **Server-side metric rollup tables / pre-computed `daily_snapshots` with diff columns** | PROJECT_SPEC.md §3.7 | 2026-04-25 |

---

## 2. Why each is out of scope

### OoS-1 — Health Assessments

The original spec proposed a periodic traffic-light form to grade each client on six axes (IP / domains / warmup / copy / funnel / server). **Decision:** the existing dashboards (DoD/WoW/MoM rates, campaign watchlist, manager portfolio with KPI progress) already give the manager the same operational signal. A separate biweekly form is busy-work without actionable lift. If health-grading is needed later, add it as a single column on `clients` rather than a new table.

### OoS-2 — CSV / Excel bulk import UI

Initial data load and ongoing batch updates are handled by **n8n / SQL**. The portal is a *thin* read+config surface. Adding a generalised import orchestration UI would require column mapping, validation pipelines, dry-runs, error tracebacks — none of which the team has bandwidth for, and none of which the agency operationally needs (the data already arrives via ingestion).

### OoS-3 — Cash flow projections

Financial planning lives outside this portal. The agency uses dedicated finance tools.

### OoS-4 — Issue tracking
...
### OoS-5 — Auto reports & exports

Stakeholders consume metrics via **dashboards**, not reports. Adding CSV/PDF export pipelines is feature creep without identified consumers. The single CSV export that does exist (Client Pipeline → Export) is a focused tool for the client to take their lead list to sales tooling.

### OoS-6 — Partnerships

Partnership tracking is not a portal concern. Out of scope.

### OoS-7 — Lost-clients ABM
...
### OoS-8 — Reply triage UI

Every reply is classified by **n8n**. The portal does not classify replies. The "Unclassified replies" count on the Admin dashboard is a sanity check on ingestion — if the count grows, n8n is not working, not "the user needs to classify". See [11-integrations §6](./11-integrations.md#6-reply-classification).

### OoS-9 — In-portal email/SMS dispatch
...
### OoS-10 — RR/BR per-team-member reports
...
### OoS-12 — Smartlead / Bison API sync from portal
...
### OoS-14 — `crm_prospects` / `lg_pipeline` separate tables
...
### OoS-15 — Server-side rollup tables
...
1. Add a **decision entry** to [BUSINESS_LOGIC.md §12](../../BUSINESS_LOGIC.md#12-decisions-log) explaining why scope changed.
...
What is in scope but **not yet built**. Single source: [BUSINESS_LOGIC.md §11](../../BUSINESS_LOGIC.md#11-open-backlog-planned-not-built). Reproduced here as a quick checklist.
...
- **BL-6** Rename "At-risk" → "Non-active clients"; optionally extend status set.
- **BL-7** Rename "Reply scope" filter → "Lead OOO scope".
...
- **BL-11** Migrate hand-written queries to Drizzle ORM.

When a backlog item ships, remove it from this list and from BUSINESS_LOGIC.md §11.
