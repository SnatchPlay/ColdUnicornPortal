...
| Reply-rate color threshold (client dashboard list) | **≥ 5 %** green, otherwise yellow | client-dashboard-page | Visual cue for "healthy" campaign on the conversion-funnel companion list. |
...
[manager-dashboard-page.tsx:50](../../../src/app/pages/manager-dashboard-page.tsx#L50) — Counts as "in progress" when both `won === false` AND `offer_sent === false`. Note: leads with `qualification = 'rejected'` or `'OOO'` still pass this test as long as those two booleans are false. This is intentional — they remain "in the funnel" until explicitly moved to a terminal state.

### "Non-active clients" surface (formerly "At-risk")

[admin-dashboard-page.tsx:77-79](../../../src/app/pages/admin-dashboard-page.tsx#L77-L79) — Hard-coded `status ∈ ('On hold', 'Offboarding', 'Sales')`. Per [BUSINESS_LOGIC §12 decision (2026-04-25)](../../BUSINESS_LOGIC.md#decision-2026-04-25-rename-at-risk-to-non-active), label is being renamed and the set may be extended to also include `Inactive`. Tracked as BL-6.

### KPI progress fallback

[clients-page.tsx:269](../../../src/app/pages/clients-page.tsx#L269) — When `client.kpi_leads === 0` (no contracted target), per-client KPI progress shows "n/a" rather than dividing by zero.

### `getLeadStage` precedence

[selectors.ts:70-77](../../../src/app/lib/selectors.ts#L70-L77) — Top-down precedence: `won → offer_sent → meeting_held → meeting_scheduled → unqualified → qualification`. A lead with `won = true` AND `qualification = 'preMQL'` displays as "won". This is the **display** stage and does not validate that the underlying booleans agree (see invariants in [BUSINESS_LOGIC §5.1](../../BUSINESS_LOGIC.md#51-lead)).

### `qualification` case sensitivity in DoD/3-DoD/WoW/MoM

[client-metrics.ts:191](../../../src/app/lib/client-metrics.ts#L191) — `lead.qualification?.toLowerCase()` is compared to lowercase literals (`"mql"`, `"premql"`). The stored enum values are `MQL` / `preMQL`; the comparison is therefore case-insensitive. Don't introduce mixed-case enum values without updating this branch.

### 3-DoD threeDodTotal includes both preMQL and MQL

[client-metrics.ts:195-202](../../../src/app/lib/client-metrics.ts#L195-L202) — `threeDodTotal` increments for `qualification ∈ {preMQL, MQL}`; `sql` (= MQL leads) increments only for `MQL`. A lead at preMQL counts toward the "total" bucket but not toward the "SQL" bucket — by design.

### Reply scope filter on lead pages

[leads-page.tsx:152-153](../../../src/app/pages/leads-page.tsx#L152-L153), [client-leads-page.tsx](../../../src/app/pages/client-leads-page.tsx) — Filters **leads by `qualification`**:

- `replyScope === "ooo"` → `qualification === "OOO"`.
- `replyScope === "active"` → `qualification !== "OOO"`.
- `replyScope === "all"` → no filter.

It does **not** filter replies by `replies.classification`. The label is misleading; rename tracked as BL-7.

### CSV export filename

[client-leads-page.tsx:152-188](../../../src/app/pages/client-leads-page.tsx#L152-L188) — `client-leads-{timeframe-label}.csv` with the timeframe label lowercased and spaces replaced by dashes (e.g. `client-leads-last-30-days.csv`). Quotes inside cells are escaped with `""`.

### Profile name split

[auth.tsx:324-362](../../../src/app/providers/auth.tsx#L324-L362) — Splits on the **first** space: `"Jan Maria Kowalski"` → `first_name = "Jan"`, `last_name = "Maria Kowalski"`. Preserves multi-word last names but discards the option of a multi-word first name.
...
[statistics-page.tsx:61-66](../../../src/app/pages/statistics-page.tsx#L61-L66) — When the user changes the client filter and the previously-selected campaign no longer belongs to a visible client, `campaignFilterId` is reset to `ALL_FILTER_VALUE`. Avoids "selected but invisible" states.

### Daily-stats inclusion gate

[core-data.tsx:103](../../../src/app/providers/core-data.tsx#L103) — `includeDailyStats = identity?.role !== "client"`. During super-admin impersonation of a client, the snapshot reload omits `daily_stats`, so client-shell pages that rely on it render empty states. Document for support; this is intentional ([10-nfr §1.1](./10-nfr.md#11-bulk-snapshot)).

### Auth state-change debounce

[auth.tsx:238](../../../src/app/providers/auth.tsx#L238) — `window.setTimeout(..., 0)` defers the auth-state listener so multiple Supabase events (TOKEN_REFRESHED + USER_UPDATED) within the same tick batch into one identity reload.
...
| **"At-risk clients"** (admin dashboard) | Implies a risk score | Just a visibility surface for `status ∈ ('On hold', 'Offboarding', 'Sales')`. Renamed to "Non-active clients" (BL-6). |
...
| **`getClientKpis().mqls`** | Looks like a stage count | `count(qualification === 'MQL')` — uses the raw qualification, not `getLeadStage`. Differs from "MQL stage count" once a lead progresses past MQL. |

---

## 4. Mutation semantics

[09-mutations-rls.md §5](./09-mutations-rls.md#5-optimistic-updates--rollback) describes the pattern; this table summarises which mutations are optimistic vs fire-and-forget.
...
Optimistic updates use the pattern: snapshot replace → repository call → on success replace with server response → on failure restore previous + toast. Mutations are **not auto-retried** even on transient failures.
...
Recovery actions on every blocker: "Retry account check" → `refreshIdentity()`; "Sign out" → `signOut()`.

`classifyAuthErrorCode(message, code)` ([auth.tsx:79-101](../../../src/app/providers/auth.tsx#L79-L101)) maps DB / network errors into the codes above using keyword matching (`permission`, `forbidden`, `denied`, `policy`, `42501` → permission; `network`, `fetch`, `timeout`, `502/503/504` → network; otherwise `unknown`).
...
End of constants reference. Next: [13 · Out of scope](./13-out-of-scope.md).
