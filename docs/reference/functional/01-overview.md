...
- **Three cooperating systems.** Bison sends and receives emails; **n8n** ingests counters/replies and dispatches notifications + OOO routing; **the portal** is a thin read+config surface on top of Supabase. The portal never sends notifications, never classifies replies, never calls Bison APIs. See [11-integrations.md](./11-integrations.md) for the full topology.
