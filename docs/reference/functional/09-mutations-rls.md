...
- **Called from:** Clients page drawer save ([clients-page.tsx:267](../../../src/app/pages/clients-page.tsx#L267)) and the inline grid cells (Status, Notes, and the **satisfaction hearts** in the Client column). The CRM integration card no longer calls it — it writes nothing to our database ([ADR-0019](../../adr/0019-crm-connections-in-postgres.md)).
- **Fields (whitelist `mapClientPatch`, [index.ts:355-381](../../../supabase/functions/orm-gateway/index.ts#L355-L382)):** `name`, `status`, `satisfaction`, `manager_id`, `min_daily_sent`, `inboxes_count`, `notification_emails`, `sms_phone_numbers`, `auto_ooo_enabled`, `setup_info`, `kpi_leads`, `kpi_meetings`, contract fields. `satisfaction` is range-checked (1..3 or null) in the request contract (`parseOrmGatewayRequest`) before the UPDATE, and again by the `clients_satisfaction_range` DB CHECK.
- **ADR-0019:** `crm_config` was removed from `mapClientPatch` and the column dropped; CRM connections live in `client_crm_connections`, written by n8n through `upsert_client_crm_connection` and never by the gateway.
