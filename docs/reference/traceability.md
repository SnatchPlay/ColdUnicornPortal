...
| 9 | The portal stores no CRM state | `clients` | — | `updateClient` no longer accepts `crm_config` | card on `/client/settings`, connect-only | — | — | `pnpm test:run` 323/323, `pnpm lint` 0 errors, `pnpm build` clean. The deferred drop applied locally: `clients` 25→24 columns, and the pre-drop column list then fails with 42703 — the hazard its header describes, demonstrated | ✅ code done; drop deferred, browser check owed |
