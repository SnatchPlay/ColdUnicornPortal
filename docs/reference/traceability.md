...
| # | Rule | Tables | RPC | Gateway action | Portal surface | Metric | n8n workflow | Test | State |
|---|---|---|---|---|---|---|---|---|---|
| 1 | A client's CRM credentials live in `client_crm_connections`, not in a spreadsheet or a Data Table | `client_crm_connections` | `upsert_client_crm_connection` | — (none, ever) | — | — | `crm-credential-intake` (to build) | `20260828b` applied on a local restore of the prod schema, 2026-08-28 | ⚠️ schema proven; nothing seeded, workflow not built |
...
| 3 | A client is matched on the workspace id, never on a name — except at intake, where only a name exists | `client_sequencers`, `client_crm_connections` | `resolve_crm_connection`, `resolve_client_for_crm_intake` | — | — | — | dispatcher `mfmMYQqK73Nsx6uO` (to rewire) | join proven read-only against all 9 live workspace ids, and locally end-to-end on a synthetic workspace | ⚠️ dispatcher not rewired |
...
| 7 | A lead that resolves no CRM is recorded, not dropped | — | `resolve_crm_connection` returns NULL | — | — | — | ⛔ `Route by CRM type` has no fallback output — [G1](n8n/defect-backlog.md#g1) | NULL proven locally for a disabled connection and for an unknown workspace | ⛔ the RPC reports it; the workflow still drops it |
...
**What's left.** Rows 7 and 8 need the intake workflow and the dispatcher rewire; row 1 needs the
seed. Rows 2, 4, 5, 6 and 9 are proven. **Nothing here has touched production** — every check above
ran on a local Postgres restored from the 2026-08-12 schema dump, with synthetic fixtures only.
