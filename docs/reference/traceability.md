...
## Process: client CRM connections

[ADR-0019](../adr/0019-crm-connections-in-postgres.md) · [ADR-0010](../adr/0010-legacy-crm-integration.md) ·
[11-integrations §CRM](functional/11-integrations.md#crm-integration)

Where a client's own CRM credentials live, and how a positive-reply lead reaches that CRM. Like the
sequencer keys above this is a **credential** move, carved out of the Sheets transition. A process
doc (`processes/crm/lead-handoff.md`) is still owed.

| # | Rule | Tables | RPC | Gateway action | Portal surface | Metric | n8n workflow | Test | State |
|---|---|---|---|---|---|---|---|---|---|
| 1 | A client's CRM credentials live in `client_crm_connections`, not in a spreadsheet or a Data Table | `client_crm_connections` | `upsert_client_crm_connection` | — (none, ever) | — | — | `crm-credential-intake` (to build) | `20260828b` + `20260828c` applied to production 2026-08-28; **all 9** connections seeded, each via a rolled-back dry run then committed. The LiveSpace mapping was settled by reading the child, not by guessing ([G4](n8n/defect-backlog.md#g4)); ws 77 is parked ([G2](n8n/defect-backlog.md#g2)) | ✅ seeded; ⚠️ workflow not rewired |
| 2 | No portal role can read those credentials | `client_crm_connections` | — | — | — | — | — | RLS with no policies **plus** revoked grants. Locally as `authenticated` and as `anon`: `select` and `truncate` both *permission denied*, all four functions *permission denied*. The revoke was added after the first local run showed Supabase's default privileges leave the table truncatable by `anon` | ✅ |
| 3 | A client is matched on the workspace id, never on a name — except at intake, where only a name exists | `client_sequencers`, `client_crm_connections` | `resolve_crm_connection`, `resolve_client_for_crm_intake` | — | — | — | dispatcher `mfmMYQqK73Nsx6uO` ✅ rewired 2026-08-28 to a single `resolve_crm_connection` call | join proven read-only against all 9 live workspace ids, locally end-to-end on a synthetic workspace, and the deployed graph re-exported and verified | ✅ |
| 4 | An ambiguous client name is never resolved by guessing | `clients` | `resolve_client_for_crm_intake` | — | — | — | `crm-credential-intake` | locally: unique name resolves, duplicate raises *2 active clients named …*, unknown raises. In prod `SalesBook` and `Testing` each match 2 non-archived clients | ✅ function; ⚠️ caller not built |
| 5 | A partial credential payload never erases a stored secret | `client_crm_connections` | `upsert_client_crm_connection` | — | — | — | `crm-credential-intake` | locally: a re-connect sending `api_key` only, with `api_secret:""` and `subdomain:null`, kept the stored secret and subdomain | ✅ |
| 6 | A parked connection does not revive on re-connect | `client_crm_connections` | `upsert_client_crm_connection` (`p_enabled = NULL` keeps) | — | — | — | `crm-credential-intake` | locally: re-connect on a disabled row left `enabled=false`; passing `true` explicitly revived it | ✅ (ws 77 is the case this exists for) |
| 7 | A lead that resolves no CRM is recorded, not dropped | — | `resolve_crm_connection` returns NULL | — | — | — | ✅ `Route by CRM type` gained a fallback output wired to `Unroutable — record and fail` — [G1](n8n/defect-backlog.md#g1) fixed 2026-08-28 | NULL proven locally for a disabled connection and for an unknown workspace; the fallback edge verified on the deployed graph | ✅ recorded, not dropped — though it fails the run rather than writing `integration_sync_runs` (needs the [E1](n8n/defect-backlog.md#e1) binding) |
| 8 | The credential write path is authenticated | — | — | — | — | — | ⛔ webhook not built yet; it must ship with header auth (cf. [security finding 10](n8n/security.md)) | — | ⛔ |
| 9 | The portal stores no CRM state | `clients` | — | `updateClient` no longer accepts `crm_config` | card on `/client/settings`, connect-only | — | — | `pnpm test:run` 323/323, `pnpm lint` 0 errors, `pnpm build` clean. The drop was applied to production 2026-08-28 after a dependency pre-flight (0 dependents) and a check that the deployed gateway names the column nowhere; `clients` went 25→24 columns and the gateway's real column list still reads | ✅ browser check still owed |

**What's left.** Row 8 — the credential intake webhook does not exist yet, so a client connecting a
CRM today still lands in Make.com and not in `client_crm_connections`. Everything else is done and
verified. **This is now live on production**: the migrations, all nine seeded connections and the
rewired dispatcher. Behavioural proof on a real positive reply is still owed.
