...
| [0018](adr/0018-gateway-outbound-automation-trigger.md) | The gateway may trigger automation, and nothing else outbound | Accepted 2026-08-07 | `orm-gateway` gains exactly one class of outbound call: triggering a repository-tracked n8n workflow, to a URL from a closed server-side list, with a shared secret the workflow verifies. It never calls a vendor — n8n holds those credentials. Authorisation is `can_manage_client` checked through RLS before the call; no credential returns to the browser; a timeout reports an unknown outcome rather than a fabricated one. Reading provisioning status stays an ordinary database read. |
