...
| [0012](adr/0012-multi-sequencer-model.md) | Multi-sequencer model (catalog + per-client credentials) | Accepted 2026-07-04 | Sequencers (EmailBison/Aimfox) are first-class: `sequencers` catalog + `client_sequencers` credentials; `campaigns`/`leads` carry `sequencer_id`. |
...
| [0018](adr/0018-gateway-outbound-automation-trigger.md) | The gateway may trigger automation, and nothing else outbound | Accepted 2026-08-07 | `orm-gateway` gains exactly one class of outbound call: triggering a repository-tracked n8n workflow, to a URL from a closed server-side list, with a shared secret the workflow is meant to verify. It never calls a vendor — n8n holds those credentials. Authorisation is `can_manage_client` checked through RLS before the call; no credential returns to the browser; a timeout reports an unknown outcome rather than a fabricated one. Reading provisioning status stays an ordinary database read. **§2 shipped in violation on 2026-08-08** — the webhooks are unauthenticated at the owner's direction; registered as `knownViolations` in both manifests, review 2026-11-30. |
| [0019](adr/0019-crm-connections-in-postgres.md) | Client CRM connections live in our Postgres | Accepted 2026-08-28 | Per-client CRM credentials move from a Google Sheet tab + an n8n Data Table into `client_crm_connections`, reached through `client_sequencers.external_workspace_id`. RLS enabled with **no policies** — `service_role` only; a status badge gets a credential-free view, never a policy. The portal stops mirroring status and `clients.crm_config` is dropped. Credentials enter only via the connect webhook → `upsert_client_crm_connection`; the dispatcher reads `resolve_crm_connection`, and a NULL is recorded as `no_connection`/`disabled`/`unknown_provider` rather than falling through. Partially supersedes ADR-0010's "tokens never reach our project". |
...
- ADR-0019 **partially supersedes** ADR-0010: its "tokens and secrets never reach our project"
  invariant no longer holds — client CRM credentials are stored in `client_crm_connections`. ADR-0010's
  read path (provider catalog, OAuth consent flows on the legacy project) stands.
