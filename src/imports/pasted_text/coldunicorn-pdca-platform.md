...
Onboarding: Створення профілю клієнта, фіксація фінансових умов та KPI (kpi_leads). Налаштування API-інтеграцій (Bison), закупівля та прив'язка пулу доменів, створення правил OOO-маршрутизації.
...
CREATE TABLE campaigns (
  id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  client_id           UUID NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
  external_id         TEXT UNIQUE, -- ID безпосередньо з секвенсера (Bison)
...
Вхідні Webhooks (Bison -> Make.com -> Supabase): * Секвенсер надсилає webhook про нову відповідь.
...
Відправляє POST-запит до Bison для додавання контакту в кампанію. У разі успіху знімає прапорець is_ooo.
